...
def connect_to_wifi(ssid,key):
    ...
    # Use wlan0 as WiFi device
    #device_path = manager.GetDeviceByIpIface("wlan0")
    
    # Use manager.GetDevices() method to obtain a list of all devices, and 
    # then iterate over these devices to check if DeviceType property 
    # equals NM_DEVICE_TYPE_WIFI (2).
    device_path=None
    dev_paths=manager.GetDevices()
    for dev_path in dev_paths:
        dev_obj=bus.get_object("org.freedesktop.NetworkManager",dev_path)
        dev_prop=dbus.Interface(dev_obj,
                                "org.freedesktop.DBus.Properties")
        dev_type=dev_prop.Get("org.freedesktop.NetworkManager.Device",
                              "DeviceType")
        if dev_type==2:
